...
#import "OYTopicViewController.h"
...
@interface OYTopicViewController ()<oYTopicCellDelegate>
/**
 *  模型数组
 */
...
@implementation OYTopicViewController
...
- (void)viewDidLoad {
    ...
    self.tableView.contentInset = UIEdgeInsetsMake(OYNavgationBarH + OYTitlesViewH,  0 , OYTarBarHeight, 0);
...
-(void)playBtnclick:(OYTopicCell *)cell{
    ...
    if (![cell.topicItem.videouri isEqualToString:@""]) {
        //        self.player = [AVPlayer playerWithURL:[NSURL URLWithString:cell.topicItem.videouri]];
...
-(void)loadNewDate1:(BOOL)boolMaxtime{
...
//
//        [self.tableView reloadData];
//        //            NSLog(@"%zd",self.itemsArray.count);
//        [self footEndRefresh];
//    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
//        [self footEndRefresh];
//        if (error.code == NSURLErrorCancelled) return;
